...
enum class ArrowTransport(
     ...
     * このアプリは SDK 0.4.0 のままなので、ファームが新しいと画像が出ない可能性がある。
     * 他の経路は出るのにこれだけ出ないなら、ファームのフレーム変更か
     * ファームが2.2.0未満のどちらか、という切り分けができる。
     */
    CANVAS_IMAGE(
        label = "キャンバス画像",
        api = "sendCanvasImage",
        note = ImageRoute.CANVAS.note + "。ファームが新しく sendCanvasImage のフレームが" +
            "変わっていると、SDK0.4.0のこのアプリでは画像が出ない可能性がある。他の経路は" +
            "出るのにこれだけ出ないなら、ファームのフレーム変更かファームが2.2.0未満の" +
            "どちらか、という切り分けになる",
        pacing = PacingPolicy.SEND_THEN_WAIT,
    ),
