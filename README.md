...
### 文字グリッドが崩れるのは折り返しに賭けているから

既定の `GridMode.WRAP` は `lines.joinToString("")` で cols×rows 文字を改行なしの
一続きにして送り、ファームが cols 文字ごとに折り返すことに賭けていた。SDK のパケットには
改行の概念もフォント寸法も無く、`elementBody` は `text.toByteArray(Charsets.UTF_8)` を
そのまま積むだけ（SDK 0.4.0 の `CanvasKey.elementBody`）。既定の 17x10 を 560px幅の枠に
入れていたが、フォント寸法の手がかりが唯一 AIチャット画面の「フォント24px+行間2px」しか
無く、そこから類推すると1行に入る文字数は cols=17 とは一致しない。結果、170文字が
意図した10行に折り返されず、段が斜めにずれていく。

直し方は2つ:

- **方式A（明示改行）**: 行間に `\n` を入れて送る。ファームが 0x0A で改行する保証は
  SDK 側に無く、実機で確認しないと使えるか分からない。改行を入れると `rows - 1` バイトぶん
  予算を消費するので、`checkBudget` はそのぶんを数えるように直した（17x10 は改行込みで
  179Bとなり173Bの予算を超えるため不可、16x10 は169Bで収まる）。
- **方式B（枠合わせ）**: 枠の幅を `cols × 1文字の送り幅` に合わせ、狙った桁でファームに
  折り返させる。KDoc の「テキストは矩形内で折り返し」という記述に沿うぶん確度が高い。

どちらも実測した1文字の送り幅・行送り・改行の可否（`CellMetrics`）が要る。「パラパラ」
タブの実測パネルで4種類を測れる: 改行が効くか、折り返し桁数（枠幅2点から線形かどうかも
分かる）、等幅か（等幅でなければ点灯・消灯の文字は送り幅が揃う組を選ぶ必要がある）、
行の高さ。測った値は保存され、次回以降スライダーを合わせ直さなくて済む。

`GridMode.ROWS`（1行1要素）は改行に依存しないぶん確実だが、要素 id が 0..7 の8個しか
無い。5行以上を送るときは `sendCanvas`（先頭4要素、CONTROL_CLEAR込み、予算137B）→
`sendCanvasElements`（残り4要素、予算142B）の2パケットに分けることで、1行あたりの
文字数を11桁から34桁まで伸ばせる。この2つはどちらも単発の `sendCommand` で、複数パケット
転送どうしの混線を防ぐ SDK 0.6.0 の `sendCommandsMutex`（このアプリは 0.4.0 のまま）の
対象にもならないため、2発の到着順序はSDK側から保証されない。後半が先に着くと前半の
CONTROL_CLEAR に消される恐れがあるので、`GlassSession.showCanvasRows` は間に
`ROWS_SPLIT_SETTLE_MS`（80ms、他の `*_SETTLE_MS` と同じ経験則）を挟んでいる。
代償は fps が単純な半減よりも重く落ちることと、2パケットの送信の合間だけ前半4行と
後半4行のコマがずれて見える瞬間があり得ること。
